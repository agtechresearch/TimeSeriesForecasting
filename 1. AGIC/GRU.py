...
# Model
model = Sequential()
model.add(GRU(units=128, return_sequences=True, recurrent_dropout=0, input_shape=(X_train.shape[1],1)))
model.add(Dropout(0.3))
model.add(GRU(units=64, recurrent_dropout=0))
model.add(Dropout(0.3))
model.add(Dense(1))
...
print("MAE:", mae)  #MAE: 0.008876991

rmse = np.sqrt(mean_squared_error(y_test, predictions))
print("RMSE: %f" % (rmse)) #RMSE: 0.013922

# model save
joblib.dump(model,'/home/jy/AGIC/agic_gru_elong.pkl')

# 모델 불러옴
# file -> model load
model_from_joblib = joblib.load('/home/jy/AGIC/agic_gru_elong.pkl')
