...
'''
Data load
: Environment 예측을 위해 Crop Parameter를 뺐음
...
gc.info()  
gc = gc[['time','Tair','Rhair', 'CO2air']]
gc.head()

w.info()  # P_output, T_sky는 전부 NaN이므로 제거한다.
w = w.drop(['P_out', 'T_sky'], axis = 1) #'Winddir', 'Pyrgeo', 'u_wind', 'Windsp', 'T_out', 'PARout'])
w
...
'''
gcw3.set_index('time', inplace = True)
scaler = MinMaxScaler()
scaler.fit(gcw3)
gcw3_scaled = scaler.transform(gcw3)
gcw3_scaled = pd.DataFrame(gcw3_scaled, columns = gcw3.columns)
gcw3_scaled.info()

gcw4 = pd.merge(a, gcw3_scaled, left_index = True, right_index = True) # datetime이 사라져 저장해 뒀던 'time_x'를 붙임
gcw4.info()
gcw4.set_index('time', inplace = True)
gcw4.head()
...
'''
# visualization
gcw4.corr(method = 'pearson')
sns.heatmap(gcw4.corr(method = 'pearson'))
...
vif["VIF Factor"] = [variance_inflation_factor(gcw4.values, i) 
for i in range(gcw4.shape[1])]
vif["features"] = gcw4.columns 
...
'''
df.to_csv('df_AGIC_final_forEnv.csv')


'''
완성된 데이터 프레임에는 총 8가지의 칼럼이 포함되어있음. 각각의 요소는 다음과 같은 의미를 가짐.
- Rhair : 온실 내 상대습도(%)
- CO2air : 온실 내 CO2 농도(ppm)
- T_out : 온실 외부 온도
- PARout : 온실 외부 PAR 측정량(umol/m2s)
- Pyrgeo : 온실 외부 heat emission(W/m2)
- RadSum : 온실 외부 radiation sum(J/cm2)
- Winddir : 온실 외부 풍향
- Windsp : 온실 외부 풍속(m/s)

=> 전처리된 데이터셋을 기반으로 온실 내 상대습도와 CO2 농도를 예측해볼 예정
'''
# %%
